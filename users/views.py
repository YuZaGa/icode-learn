...
def kerkesa(request):
    if request.method == 'POST':
        ...
        kerkesa = Kerkesat(profili=prof, emri=emri,
                           email=email, numri_tel=numri_tel)
        kerkesa.save()
        prof_id = prof.id
        ...
        send_mail(
            'Asamkhyeya, the request was accepted.',
            message,
            'asamkhyeya@no-reply.com',
            [email],
            fail_silently=False,
        )
        send_mail(
            'Asamkhyeya',
            'Someone made a request on behalf of the teacher. With info:' +
            emri + ' , ' + email + ' , ' + numri_tel + ' , ' + str(prof) + '.',
            'asamkhyeya@no-reply.com',
            ['contact@sagarrathi.com'],
            fail_silently=False,
        )
        messages.info(
            request, f'The request was successfully sent, you will be notified by email.')
